...
RC RecordBasedFileManager::insertRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, RID &rid) 
{
    ...
    // Iterate through pages to find space for the new record.
    ...
    {
        if (fileHandle.readPage(pageIndex, pageData))
            return RBFM_READ_FAILED;

        // If the page has enough space for the new record, stop iterating.
    ...
    // If no page with enough space is found, create a new one.
    if (!pageFound)
    {
    ...
    // Add the new record reference to the slot directory.
    ...
    // Update the slot directory header.
    ...
    if (pageFound)
    {
        if (fileHandle.writePage(pageIndex, pageData))
...
{
    // Allocate memory to retrieve the specific page
    void *pageData = malloc(PAGE_SIZE);
    if (pageData == NULL)
        return RBFM_MALLOC_FAILED;
    ...
    // Determine the status of the record entry
    SlotStatus status = getSlotStatus(recordEntry);
    switch (status)
    {
        // Error: attempting to read a deleted record
        case DEAD:
            free(pageData);
...
RC RecordBasedFileManager::deleteRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid)
{
    ...
    // Get the slot record entry data
    SlotDirectoryRecordEntry recordEntry = getSlotDirectoryRecordEntry(pageData, rid.slotNum);
    SlotStatus status = getSlotStatus(recordEntry);

    // Cannot delete a record that is already marked as deleted
    if (status == DEAD)
    {
        free(pageData);
        return RBFM_SLOT_DN_EXIST;
    }
    // Recursively delete moved records
    ...
    // Once the record(s) have been deleted, write changes to disk
...
RC RecordBasedFileManager::updateRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, const RID &rid)
{
    ...
    switch (status)
    {
        // Error: attempting to update a deleted record
        case DEAD:
            free(pageData);
            return RBFM_READ_AFTER_DEL;
        // Get the forwarding address from the record entry and recurse
    ...
    // Get the size of the updated record
    unsigned recordSize = getRecordSize(recordDescriptor, data);
    ...
    for (unsigned i = 0; i < recordDescriptor.size(); i++)
    {
        ...
        switch (recordDescriptor[i].type)
        {
            case TypeInt:
                ...
                memcpy(&data_integer, static_cast<const char*>(data) + offset, INT_SIZE);
                offset += INT_SIZE;
            ...
            case TypeReal:
                float data_real;
                memcpy(&data_real, static_cast<const char*>(data) + offset, REAL_SIZE);
                offset += REAL_SIZE;
            ...
            case TypeVarChar:
                ...
                char *data_string = static_cast<char*>(malloc(varcharSize + 1));
                if (data_string == NULL)
                    return RBFM_MALLOC_FAILED;
                memcpy(data_string, static_cast<const char*>(data) + offset, varcharSize);
...
{
    char *pageData = static_cast<char*>(malloc(PAGE_SIZE));
    if (pageData == NULL)
        return RBFM_MALLOC_FAILED;
    if (fileHandle.readPage(rid.pageNum, pageData) != SUCCESS)
    {
...
{
    if (fileHandle.readPage(currPage, pageData) != SUCCESS)
        return RBFM_READ_FAILED;
...
SlotDirectoryRecordEntry RecordBasedFileManager::getSlotDirectoryRecordEntry(void *page, unsigned recordEntryNumber)
{
    ...
    memcpy(&recordEntry, ((char *)page + sizeof(SlotDirectoryHeader) + recordEntryNumber * sizeof(SlotDirectoryRecordEntry)), sizeof(SlotDirectoryRecordEntry));
    return recordEntry;
}
...
void RecordBasedFileManager::setSlotDirectoryRecordEntry(void *page, unsigned recordEntryNumber, SlotDirectoryRecordEntry recordEntry)
{
    ...
    memcpy(((char*)page + sizeof(SlotDirectoryHeader) + recordEntryNumber * sizeof(SlotDirectoryRecordEntry)), &recordEntry, sizeof(SlotDirectoryRecordEntry));
}
...
unsigned RecordBasedFileManager::getRecordSize(const vector<Attribute> &recordDescriptor, const void *data)
{
    ...
    for (unsigned i = 0; i < (unsigned)recordDescriptor.size(); i++)
    {
        ...
        switch (recordDescriptor[i].type)
        {
            ...
            case TypeVarChar:
                uint32_t varcharSize;
                // Get the size of the VarChar field by reading the integer that precedes the string value itself
...
void RecordBasedFileManager::setRecordAtOffset(void *page, unsigned offset, const vector<Attribute> &recordDescriptor, const void *data)
{
    ...
    char nullIndicator[nullIndicatorSize];
    memset(nullIndicator, 0, nullIndicatorSize);
    memcpy(nullIndicator, static_cast<const char*>(data), nullIndicatorSize);

    // Points to start of record
    char *start = static_cast<char*>(page) + offset;
    ...
    for (unsigned i = 0; i < recordDescriptor.size(); i++)
    {
        if (!fieldIsNull(nullIndicator, i))
        {
            ...
            const char *data_start = static_cast<const char*>(data) + data_offset;
...
void RecordBasedFileManager::getRecordAtOffset(void *page, int32_t offset, const vector<Attribute> &recordDescriptor, void *data)
{
    ...
    for (unsigned i = 0; i < recordDescriptor.size(); i++)
    {
        ...
        {
            memcpy(static_cast<char*>(data) + data_offset, &fieldSize, VARCHAR_LENGTH_SIZE);
            data_offset += VARCHAR_LENGTH_SIZE;
        }
        // Next we copy bytes equal to the size of the field and increase our offsets
        memcpy(static_cast<char*>(data) + data_offset, start + rec_offset, fieldSize);
...
{
    memset(static_cast<char*>(page) + sizeof(SlotDirectoryHeader) + i * sizeof(SlotDirectoryRecordEntry), 0, sizeof(SlotDirectoryRecordEntry));
}
...
void RecordBasedFileManager::reorganizePage(void *page)
{
    ...
    for (unsigned i = 0; i < liveRecords.size(); i++)
    {
        ...
        memmove(static_cast<char*>(page) + pageOffset, static_cast<char*>(page) + current.offset, current.length);
...
void RecordBasedFileManager::getAttributeFromRecord(void *page, unsigned offset, unsigned attrIndex, AttrType type, void *data)
{
    ...
    if (type == TypeVarChar)
    {
        ...
        memcpy(static_cast<char*>(data) + data_offset, &len, sizeof(VARCHAR_LENGTH_SIZE));
        data_offset += VARCHAR_LENGTH_SIZE;
    }
    // For all types, we then copy the data into the result
    memcpy(static_cast<char*>(data) + data_offset, start + attrStart, len);
}
